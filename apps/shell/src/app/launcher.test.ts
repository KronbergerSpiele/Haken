...
describe('launcher', () => {
  ...
  it('renders metadata and actions for every registered game', async () => {
  ...
  it('shows loading and failure states on the play button', async () => {
    await renderLauncher(
      root,
      {
        manifests: CATALOG,
  ...
  it('calls play and share handlers from card actions', async () => {
    const onPlay = vi.fn();
    const onShare = vi.fn();
    await renderLauncher(
      root,
