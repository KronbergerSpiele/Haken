...
import { cardBackTemplate, cardFaceTemplate } from './graphics';
...
export class ZoffDragController {
  ...
  private createPreview(source: PileDragSource): HTMLElement {
    ...
    this.renderPreviewContent(preview, source);
    return preview;
  }

  private renderPreviewContent(preview: HTMLElement, source?: PileDragSource): void {
    const species = this.callbacks.getPendingSpecies();
    if (species) {
      litRender(cardFaceTemplate(species, { compact: true }), preview);
    } else if (source === 'deck') {
      litRender(cardBackTemplate({ compact: true }), preview);
    } else {
      litRender(null, preview);
    }
  }
