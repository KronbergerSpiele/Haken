...
import type { SessionContext } from '@spiele/engine/contracts';
import { createSeededRandom } from '@spiele/engine/random';
import { createEffectService } from '@spiele/graphics/effects';
import { createFeedbackService } from '@spiele/graphics/feedback';
import { createGraphicsService } from '@spiele/graphics/primitives';
import { createInputService } from '@spiele/engine/input';
import { RuntimeClock } from '@spiele/engine/runtime';
...
describe('zoff session', () => {
  ...
  it('mounts with theme scope and shared collection exit element', () => {
    ...
    expect(root.querySelector('spiele-collection-exit')).not.toBeNull();
  ...
  it('reuses the shared collection exit element across redraws', () => {
    session = new ZoffSession(createContext()) as ZoffSession;
    session.mount(root);
    const exit = root.querySelector('spiele-collection-exit');
    root.querySelector<HTMLButtonElement>('[data-start]')!.click();
    expect(root.querySelector('spiele-collection-exit')).toBe(exit);
  });

  it('requests collection exit from the shared exit element', () => {
