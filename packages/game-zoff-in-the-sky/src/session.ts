...
import { applyZoffTokens } from './theme';
...
export class ZoffSession implements GameSession {
  ...
  private readonly exitHandler: (event: Event) => void;

  constructor(private readonly context: SessionContext) {
    this.game = createGame(context.seed);
    this.clickHandler = (event) => this.handleClick(event);
    this.keyHandler = (event) => this.handleKey(event);
    this.exitHandler = () => this.context.requestExit();
  }
  ...
  mount(root: HTMLElement): void {
    ...
    root.addEventListener('spiele-exit', this.exitHandler);
  ...
  dispose(): void {
    ...
    this.root?.removeEventListener('spiele-exit', this.exitHandler);
  ...
  private ensureCollectionExit(): void {
    if (!this.root) return;
    mountCollectionExit(this.root);
  }
