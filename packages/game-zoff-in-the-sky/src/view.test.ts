...
import { SPIELE_ZOFF_VIEW_TAG } from './view-element';
import { eatingOverlaysTemplate } from './view-templates';
import {
...
function renderTemplate(template: Parameters<typeof litRender>[0]): HTMLElement {
...
describe('zoff graphics helpers', () => {
  ...
  it('renders eating indicators as sprite icons without abbreviation text', () => {
    const container = renderTemplate(eatingIndicatorsTemplate('fox'));
    expect(container.querySelector('.zoff-eat-icon')).not.toBeNull();
    expect(container.textContent).not.toContain('>Fu<');
    expect(container.querySelector('.zoff-eat-glyph')).toBeNull();
    expect(container.querySelector('[role="img"]')).not.toBeNull();
    expect(container.querySelector('.zoff-eat-icon__art')).not.toBeNull();
  });
  ...
  it('renders multiple simultaneous chain groups with row labels', () => {
    ...
    expect(container.querySelectorAll('.zoff-eating-overlay')).toHaveLength(2);
    expect(container.textContent).toContain('Reihe 1');
    expect(container.textContent).toContain('Reihe 3');
    expect(container.textContent).toContain('Mücke → Maus → Fuchs');
  });
});
...
describe('zoff view rendering', () => {
  ...
  it('hosts content in a layout-neutral Lit view element', () => {
    render(root, createGame(12), INITIAL_UI);
    const host = root.querySelector(SPIELE_ZOFF_VIEW_TAG);
    expect(host).not.toBeNull();
    expect((host as HTMLElement).style.display).toBe('contents');
    expect(root.querySelector('h1')?.textContent).toBe('Zoff in the Sky');
  });

  it('reuses the same view element across redraws', () => {
    const game = createGame(12);
    render(root, game, INITIAL_UI);
    const firstHost = root.querySelector(SPIELE_ZOFF_VIEW_TAG);
    render(root, transition(game, { type: 'start' }).state, INITIAL_UI);
    expect(root.querySelector(SPIELE_ZOFF_VIEW_TAG)).toBe(firstHost);
  });
  ...
  it('places final eating overlay above the result layer', () => {
    ...
    expect(shell).not.toBeNull();
    const overlayContainer = renderTemplate(
      eatingOverlaysTemplate(game, {
        ...INITIAL_UI,
        eatingOverlayChains: [{ player: 0, row: 0, species: ['mosquito', 'mouse', 'fox'] }],
      }),
    );
    expect(overlayContainer.querySelector('.zoff-eating-overlays--above-result')).not.toBeNull();
